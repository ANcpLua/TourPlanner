...
public class RouteApiService(HttpClient httpClient, IConfiguration configuration) : IRouteApiService
{
    [UiMethodDecorator]
    ...
    )
    {
        ...
        var response = await httpClient.PostAsync($"{baseUrl}/v2/directions/{endpoint}", content);
